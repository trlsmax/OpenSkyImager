...
GSList *script_parameter_list = NULL;
...
#define BUF_SIZE 100
    ...
    for(i = 0; i < line_cnt; i++) {
        ...
        *script_ptr = '\0';
        ...
        if(script[0] == '\0') {
            ...
            if(num <= 0 || (bin < 0 || bin > 4) || (gain < 0 || gain > 100) || (offset < 0 || offset > 255)) {
                snprintf(str, BUF_SIZE, "Line:%2d, ERROR, Please check!\n", i);
                gtk_text_buffer_insert_with_tags_by_name(output, &iter, str, -1, "red_bg", NULL);
                continue;
            }

            script_parameter_list = g_slist_append(script_parameter_list, g_new0(struct script_parameter, 1));
            list_ptr = g_slist_last(script_parameter_list);
            ((struct script_parameter *)list_ptr->data)->num = num;
            ((struct script_parameter *)list_ptr->data)->start_from = total;
            ((struct script_parameter *)list_ptr->data)->exposure = exposure;
            ((struct script_parameter *)list_ptr->data)->bin = bin;
            ((struct script_parameter *)list_ptr->data)->gain = gain;
            ((struct script_parameter *)list_ptr->data)->offset = offset;
            total += num;

            snprintf(str, BUF_SIZE, "Line:%2d, num=%d, exposure=%d, bin=%d, gain=%d, offset=%d\n", i, num, exposure, bin, gain, offset);
            gtk_text_buffer_insert(output, &iter, str, -1);
        } else {
            snprintf(str, BUF_SIZE, "Line:%2d, ERROR, Please check!\n", i);
            gtk_text_buffer_insert_with_tags_by_name(output, &iter, str, -1, "red_bg", NULL);
        }
    ...
    snprintf(str, BUF_SIZE, "Total number of shoot is : %d\n", total);
    gtk_text_buffer_insert(output, &iter, str, -1);
